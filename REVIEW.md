...
Files fixed:
- `backend/src/social_graph/collector.py` ✅
- `backend/src/social_graph/frame_builder.py` ✅
- `backend/src/social_graph/models.py` ✅ (already had utc_now helper)
...
1. **Force-directed layout** - O(n²) but bounded by MAX_NODES (2000) ✅
2. **Database queries** - Could benefit from eager loading (minor optimization)
3. **Frontend** - Good React Three Fiber usage with proper memoization ✅
4. **API resilience** - Retry logic prevents cascade failures ✅
...
| Criterion | Status |
|-----------|--------|
| Collector produces interval and frame for 14 runs | ✅ Ready |
| Timeline scrub <10% position discontinuities | ✅ Position persistence + interpolation |
| Post click returns attribution | 🔲 M3 |
| Rebuild reproduces identical frame | ✅ Deterministic with config_hash |
